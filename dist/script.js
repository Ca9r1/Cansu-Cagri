...
shareButton.addEventListener('click', async () => {
  ...
  try {
    ...
    if (navigator.canShare?.({ files: [file] })) {
      await navigator.share({ title: 'Cansu & Çağrı — Düğün Davetiyesi', text: '31 Ekim 2026, 18.30 · Suare Event Tuzla', files: [file] });
      return;
    }
    if (navigator.share) {
      await navigator.share({ title: 'Cansu & Çağrı — Düğün Davetiyesi', text: '31 Ekim 2026, 18.30 · Suare Event Tuzla', url: window.location.href });
      return;
    }
    await navigator.clipboard.writeText(window.location.href);
    shareStatus.textContent = 'Davet bağlantısı kopyalandı.';
  } catch (error) {
